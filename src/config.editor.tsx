...
import { DataSourcePluginOptionsEditorProps, DataSourceJsonData, updateDatasourcePluginJsonDataOption, SelectableValue } from '@grafana/data';
    ...
    return <>
        <div className="gf-form-inline">
            <div className="gf-form">
                ...
                <Select
                    ...
                    value={DATASOURCE_MODES.find((field: SelectableValue) => field.value === options.jsonData.datasource_mode) || DEFAULT_DATASOURCE_MODE}
        ...
        }
        {
            (options.jsonData.global_queries && options.jsonData.global_queries.length === 0) ? (<>
                <button className="btn btn-primary" onClick={addGlobalQuery}>Add Global Query</button>
            </>) : (
                    <>
                        <h3>Registered / Global Queries</h3>
                        {
                            options.jsonData.global_queries && options.jsonData.global_queries.map((q: GlobalInfinityQuery, index: number) => <>
                                <hr />
                                <div className="gf-form-inline">
                                    <div className="gf-form">
                                        <label className="gf-form-label query-keyword width-8">Query Name</label>
                                        <input type="text" value={q.name} className="gf-form-label width-12" onChange={(e) => {
                                        ...
                                        <input type="text" value={q.id} className="gf-form-label width-12" onChange={(e) => {
                                            set(options, `jsonData.global_queries[${index}].id`, e.target.value);
                                            updateDatasourcePluginJsonDataOption(props, "global_queries", options.jsonData.global_queries)
                                        }} ></input>
                                        <span className="btn btn-danger" onClick={() => deleteGlobalQuery(index)} >Delete Query</span>
                                    </div>
                                    <div className="gf-form">
                                        <InfinityQueryEditor query={q.query} mode="global" onChange={() => updateDatasourcePluginJsonDataOption(props, "global_queries", options.jsonData.global_queries)} instanceSettings={options} />
                                    </div>
                                </div>
                            </>)
                        }
