...
export const TypeChooser: React.FC<TypeChooserProps> = ({ query, onChange, mode, instanceSettings }) => {
    ...
    let global_queries = instanceSettings && instanceSettings.jsonData && instanceSettings.jsonData.global_queries && instanceSettings.jsonData.global_queries.length > 0 ? instanceSettings.jsonData.global_queries : [];
                    ...
                    query.type === 'global' ? <>
                        {
                            global_queries.length > 0 ? <>
                                <Select
                                    ...
                                    value={global_queries.find((q: SelectableValue) => q.value === query.global_query_id) || global_queries[0]}
                                    onChange={(e) => { onSelectChange(e, 'global_query_id') }}
                                />
                        ...
                        <Select
                            ...
                            options={SCRAP_QUERY_SOURCES.filter((field: SelectableValue) => field.supported_types.indexOf(query.type) > -1)}
