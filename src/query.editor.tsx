...
export const InfinityQueryEditor: React.FC<InfinityEditorProps> = ({ query, onChange, mode, instanceSettings }) => {
    ...
    return (
        <div>
            ...
            {['csv', 'html', 'json', 'graphql'].indexOf(query.type) > -1 ? <ScrapperOptions onChange={onChange} query={query} /> : <></>}
...
export const QueryEditor: React.FC<EditorProps> = (props) => {
    ...
    return (
        <div>
            <InfinityQueryEditor onChange={onChange} query={query} mode="standard" instanceSettings={props.datasource.instanceSettings} />
        </div >
    );
}
