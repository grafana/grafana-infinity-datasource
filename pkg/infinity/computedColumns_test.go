...
func TestGetFrameWithComputedColumns(t *testing.T) {
	tests := []struct {
		name    string
		input   *data.Frame
		columns []querySrv.InfinityColumn
		output  *data.Frame
	}{
		{
			input:   data.NewFrame("sample", data.NewField("id", nil, []*float64{toFP(1), toFP(2)})),
			columns: []querySrv.InfinityColumn{{Selector: "id + id", Text: "twice"}},
		...
		},
		{
			...
			columns: []querySrv.InfinityColumn{{Selector: "[Cylinders] + horsepower", Text: "power"}},
