...
Bearer tokens enable requests to authenticate using an access key, such as a JSON Web Token (JWT) or a personal access token.
...
{{< admonition type="tip" >}}
It is easy to confuse API key authentication with bearer token authentication, make sure you are using the correct authentication mechanism.
{{< /admonition >}}

## Digest

Digest authentication enables requests to authenticate using [RFC7616 HTTP Digest Access Authentication protocol](https://www.rfc-editor.org/rfc/rfc7616.txt).
