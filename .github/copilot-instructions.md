...
- **Backend tests**: Use `*_test.go` co-located with source; use `testify` for assertions; prefer table-driven tests
...
Workflow skills are available in `.github/skills/` and can be invoked with `/` in chat:

| Skill | When to use |
|-------|-------------|
| `add-auth-type` | Implementing a new authentication method (frontend + backend) |
